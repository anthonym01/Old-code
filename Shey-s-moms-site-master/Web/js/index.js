window.addEventListener('load', function () {
    console.warn("Please dont cheat friend")
    /*  console.log('⣿⣿⣿⣿⣿⣿⣿⣿⣿⠟⠛⢉⢉⠉⠉⠻⣿⣿⣿⣿⣿⣿')
        console.log('⣿⣿⣿⣿⣿⣿⣿⠟⠠⡰⣕⣗⣷⣧⣀⣅⠘⣿⣿⣿⣿⣿')
        console.log('⣿⣿⣿⣿⣿⣿⠃⣠⣳⣟⣿⣿⣷⣿⡿⣜⠄⣿⣿⣿⣿⣿')
        console.log('⣿⣿⣿⣿⡿⠁⠄⣳⢷⣿⣿⣿⣿⡿⣝⠖⠄⣿⣿⣿⣿⣿')
    ...
    config.initialize()//Get\make local config
    testmanger.initalize()//start up the torture machine
    Ui.initialize()
})
...
let config = {
    data: {//Loacal app data
        ...
        theme: "devicebased",
        accent_color: -1,
        loggin: { state: false, id: null, username: null },
    },
    ...
    validate: function () {//validate configuration file
        ...
        if (typeof (config.data.loggin) == 'undefined') {
            config.data.loggin = { state: false, id: null, username: null }
            configisvalid = false
            console.log('"loggin state" did not exist and was set to default')
        }
...
let testmanger = {
    ...
    data: [
        {
            name: "Test Test", details: "A test to test the test system by testing a  fake test", questions: [
                { question_txt: "Did the test view start?", answers: ["yes", "no"], desired_ans: 0 }
            ], results: null,
        }
    ],
    initalize: function () {
        this.render_test_anchors()
    },
    render_test_anchors: function () {
        console.log('Building test anchors')
        document.getElementById('test_select_container').innerHTML = ""//Clear test selector
        for (i = 0; i < testmanger.data.length; i++) {
            build_anchor(i);
        }

        function build_anchor(index) {
            console.log('Building anchor: ', index);
            //Build exam_buttons
            var exam_pedistal = document.createElement('div')
            var pediastal_face = document.createElement('div')
            var title = document.createElement('div')
            var details = document.createElement('div')
            details.classList = "details"
            details.innerHTML = testmanger.data[index].details
            title.classList = "title"
            title.innerHTML = testmanger.data[index].name
            exam_pedistal.classList = "exam_pedistal"
            exam_pedistal.title = "Select " + testmanger.data[index].name
            pediastal_face.classList = "pediastal_face"
            exam_pedistal.appendChild(pediastal_face)
            pediastal_face.appendChild(title)
            pediastal_face.appendChild(details)

            document.getElementById('test_select_container').appendChild(exam_pedistal)
            exam_pedistal.addEventListener('click', function () {//click exam button to select test
                console.log('Clicked test anchor: ', index);
                testmanger.select_test(index);
            })

        }
    },
    select_test: function (testid) {
        console.log('Preping test: ', testid)
        document.getElementById('test_taker').classList = "mainview_active"
        document.getElementById('test_selector').classList = "mainview"
        document.getElementById('titleselector').innerHTML = testmanger.data[testid].name
        testmanger.properties.current_test = testid

        //preset first question
        document.getElementById('Questin_box').innerHTML = "1. " + testmanger.data[testid].questions[0].question_txt;
        var answer_bar = document.createElement('div')
        answer_bar.classList = "answer_bar"
        var answer_text = document.createElement('div')
        answer_text.classList = "answer_text"
        answer_text.innerHTML = testmanger.data[testid].questions[0].answers
    },
...
let Ui = {
    initialize: function () {//start ui logic
        ...
        if (config.data.loggin.state == true) {
            document.getElementById('login_Screen').style.display = 'none'
        }
        document.getElementById('liginbtn').addEventListener('click', function () {
            document.getElementById('login_Screen').style.display = 'none'
            config.data.loggin.state = true;
            config.save()
        })
        document.getElementById('body').addEventListener('click', function () {
            if (document.getElementById('setting_pannel').classList != "setting_pannel") {
                Ui.close_setting_pannel()
            }
        })
        document.getElementById('setting_pannel').addEventListener('click', function () { event.stopPropagation() })

        document.getElementById('usericon_anchor').addEventListener('click', function () {
            console.log('User anchor trigger')
            event.stopPropagation()
            if (document.getElementById('setting_pannel').classList == "setting_pannel") {
                Ui.open_setting_pannel()
            } else {
                Ui.close_setting_pannel()
            }
        })
        ...
        document.getElementById('logout_button').addEventListener('click', function () {
            config.data.loggin.state = false;
            config.save()
            location.reload()
        })
    ...
    open_setting_pannel: function () {
        console.log('open setting pannel')
        document.getElementById('setting_pannel').classList = "setting_pannel_active"
    },
    close_setting_pannel: function () {
        console.log('close setting pannel')
        document.getElementById('setting_pannel').classList = "setting_pannel"
    },
    setting: {
        set_theme: function () {//determines which theme to use
            ...
            function set_device() {
                document.getElementById('hue' + config.data.accent_color + '-selec').classList = "accent_blob_active"
                switch (config.data.accent_color) {
                    case -1:
                        ...
                        break;
                    case 0:
                        document.getElementById('body').classList = "_0";
                        console.log('%cdark _0', "color: hsl(0,100%,50%)")
                        break;
                    case 30:
                        document.getElementById('body').classList = "_30";
                        console.log('%cdark _30', "color: hsl(30,100%,50%)");
                        break;
                    case 60:
                        document.getElementById('body').classList = "_60";
                        console.log('%cdark _60', "color: hsl(60,100%,50%)");
                        break;
                    case 90:
                        document.getElementById('body').classList = "_90";
                        console.log('%cdark _90', "color: hsl(90,100%,50%)");
                        break;
                    case 120:
                        document.getElementById('body').classList = "_120";
                        console.log('%cdark _120', "color: hsl(120,100%,50%)");
                        break;
                    case 150:
                        document.getElementById('body').classList = "_150";
                        console.log('%cdark _150', "color: hsl(150,100%,50%)");
                        break;
                    case 180:
                        document.getElementById('body').classList = "_180";
                        console.log('%cdark _180', "color: hsl(180,100%,50%)");
                        break;
                    case 210:
                        document.getElementById('body').classList = "_210";
                        console.log('%cdark _210', "color: hsl(210,100%,50%)");
                        break;
                    case 240:
                        document.getElementById('body').classList = "_240";
                        console.log('%cdark _240', "color: hsl(240,100%,50%)");
                        break;
                    case 270:
                        document.getElementById('body').classList = "_270";
                        console.log('%cdark _270', "color: hsl(270,100%,50%)");
                        break;
                    case 300:
                        document.getElementById('body').classList = "_300";
                        console.log('%cdark _300', "color: hsl(300,100%,50%)");
                        break;
                    case 330:
                        document.getElementById('body').classList = "_330";
